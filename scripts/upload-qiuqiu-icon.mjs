...
async function uploadQiuqiuIcon() {
  console.log('🚀 开始上传秋秋图标...\n');

  const imageDir = path.join(__dirname, '../image');
  const imagePath = path.join(imageDir, 'e.jpg');
  ...
  console.log(`📁 上传 e.jpg...`);
  const url = await uploadFile(imagePath, 'icons/qiuqiu-icon.jpg');
...
uploadQiuqiuIcon().catch(console.error);
